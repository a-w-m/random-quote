...
async function main() {
  ...
  async function getRandomPeoplePage() {
    ...
    let peoplePagesQuery =
      "https://en.wikiquote.org/w/api.php?origin=*&action=query&generator=categorymembers&gcmtitle=Category:People&gcmnamespace=0&gcmlimit=500&format=json&formatversion=2";
    ...
    const res = {
      pageid: validPage[
        Math.floor(Math.random() * validPage.length)
      ].toString(),
  ...
  async function getRandomPersonPages(data) {
    ...
    personPages = personPages.filter(item => {
      if (
        "pageid" in item &&
        data.idsToFilter.includes(item.pageid) === false
      ) {
        return item;
      }
    });
  ...
  async function getQuotePage(personPages) {
    ...
    const pageID = personPages[Math.floor(Math.random() * personPages.length)][
      "pageid"
    ].toString();

    const stubQuery =
      "https://en.wikiquote.org/w/api.php?action=query&format=json&origin=*&prop=templates&pageids=" +
      pageID;
    ...
    try {
      ...
      for (let x = 0; x < stubJSON.query.pages[pageID].templates.length; x++) {
        if (
          stubJSON.query.pages[pageID].templates[x]["title"]
            .toLowerCase()
            .includes("stub")
        ) {
      ...
      if ("error" in responseJSON) {
        ...
        responseJSON.parse.sections[0].anchor.toLowerCase() != "quotes"
      ) {
  ...
  async function getQuote(jsonObject) {
    ...
    try {
      ...
      const verifierString =
        "[\\[\\]\\^\\+\\{\\}\\=]|\\d{4}|^\\d+?\\d+$|\\d+\\)?$|\\d+\\.\\d+?|\\d+\\w|\\W{4,}|1.Quotes|\\[edit\\]|Reffering to|p+[g]?\\.\\s?(?=\\d+|[xivcml]+)|page[s]?\\s?\\d+|ch\\.\\s?(?=\\d+|[xivcml]+)|chapter\\.?\\s(?=\\d+|[xivcml]+)|Book\\.?\\s?(?=\\d+|[xivcml]+)|line+s?\\.?\\s?(?=\\d+|[xivcml]+)|Vol(ume)?\\.|paragraph\\s?(?=\\d+|[xivcml]+)|edition|source needed|stanza|canto|Act,?\\s?scene|quoted|from the album|www|" +
        title;
      const verifier = new RegExp(verifierString, "mi");

      //apply regex to clean up and validate quotes
      let quotes = JSON.stringify(jsonObject.parse.text["*"])
        .match(grabDirtyQuotes)
  ...
  function updateSocialMediaLinks() {
    ...
    document
      .getElementById("tweet-link")
      .setAttribute(
        "href",
        "http://twitter.com/intent/tweet?text=" +
          encodeURI(document.getElementById("text").innerHTML) +
          encodeURI(document.getElementById("author").innerHTML)
      );

    document
      .getElementById("tumblr-link")
      .setAttribute(
        "href",
        "https://www.tumblr.com/widgets/share/tool?posttype=quote&tags=quotes&caption=" +
          ...
          "&content=" +
          encodeURI(document.getElementById("text").innerHTML) +
          "&canonicalUrl=https%3A%2F%2Fwww.tumblr.com%2Fbuttons&shareSource=tumblr_share_button&posttype=quote"
      );
  }
  ...
  function navigation() {
    ...
    document.getElementById("prev-quote").onclick = function() {
      document.getElementById("text").innerHTML =
        quoteMachine.previous[quoteMachine.lastIndex]["quote"];
      document.getElementById("author").innerHTML =
        "—" + quoteMachine.previous[quoteMachine.lastIndex]["author"];
